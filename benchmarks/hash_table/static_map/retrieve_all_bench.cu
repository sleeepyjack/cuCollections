...
void retrieve_all_bench(nvbench::state& state, nvbench::type_list<Pair, nvbench::enum_type<Dist>>)
{
  ...
  thrust::device_vector<key_type> result_keys(num_keys);
  thrust::device_vector<mapped_type> result_payloads(num_keys);
  ...
  state.exec(nvbench::exec_tag::sync | nvbench::exec_tag::timer,
             [&](nvbench::launch& launch, auto& timer) {
               ...
               timer.start();
               map.retrieve_all(result_keys.begin(), result_payloads.begin(), launch.get_stream());
               timer.stop();
             });
}

NVBENCH_BENCH_TYPES(retrieve_all_bench,
                    NVBENCH_TYPE_AXES(defaults::pair_type_list,
                                      nvbench::enum_type_list<dist_type::UNIQUE>))
  .set_name("cuco::static_map::retrieve_all [occupancy]")
